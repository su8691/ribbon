...
def main(args):
    ...
    img = render(canvas_width, # width
                 ...
                 None,
                 *scene_args)
    pydiffvg.imwrite(img.cpu(), 'results/painterly_rendering_noto'+ str(num_paths) + ' ' + str(max_width) + '/init.png', gamma=gamma)
    ...
    for t in range(args.num_iter):
        ...
        pydiffvg.imwrite(img.cpu(), 'results/painterly_rendering_noto' + str(num_paths) + ' ' + str(max_width) +  '/iter_{}.png'.format(t), gamma=gamma)
        img = img[:, :, :3]
        ...
        if t % 10 == 0 or t == args.num_iter - 1:
            pydiffvg.save_svg('results/painterly_rendering_noto'+ str(num_paths) + ' ' + str(max_width) + '/iter_{}.svg'.format(t),
                              canvas_width, canvas_height, shapes, shape_groups)
    ...
    img = render(target.shape[1], # width
                 ...
                 *scene_args)
    # Save the intermediate render.
    pydiffvg.imwrite(img.cpu(), 'results/painterly_rendering_noto'+ str(num_paths) + ' ' + str(max_width) + '/final.png'.format(t), gamma=gamma)
    # Convert the intermediate renderings to a video.
    from subprocess import call
    call(["ffmpeg", "-framerate", "24", "-i",
        "results/painterly_rendering_noto"+ str(num_paths) + ' ' + str(max_width) + "/iter_%d.png", "-vb", "20M",
        "results/painterly_rendering_noto"+ str(num_paths) + ' ' + str(max_width) + "/out.mp4"])
