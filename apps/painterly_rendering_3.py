...
def main(args, fileName):
    ...
    img = render(canvas_width, # width
                 ...
                 None,
                 *scene_args)
    #pydiffvg.imwrite(img.cpu(), 'results/'+ fileName[0:-4] + '.png', gamma=gamma)
    ...
    for t in range(args.num_iter):
        ...
        #pydiffvg.imwrite(img.cpu(), 'results/'+fileName+'_iter_{}.png'.format(t), gamma=gamma)
        img = img[:, :, :3]
        ...
        if  t == args.num_iter - 2:
            pydiffvg.save_svg('results/U+'+ fileName[0:-4] +'.svg'.format(t),
                              canvas_width, canvas_height, shapes, shape_groups)
    ...
    img = render(target.shape[1], # width
                 ...
                 *scene_args)
    # Save the intermediate render.
    #pydiffvg.imwrite(img.cpu(), 'results/painterly_rendering_noto/'+ fileName + '/final.png'.format(t), gamma=gamma)
    # Convert the intermediate renderings to a video.
    from subprocess import call
   # call(["ffmpeg", "-framerate", "24", "-i",
    #   "results/"+ fileName + "_iter_%d.png", "-vb", "20M",
     #   "results/"+ fileName + "_out.mp4"])

if __name__ == "__main__":
    start = time.time()
    total_start = start
    yourPath =  "/mnt/hgfs/font/imageWH"
    ...
    for file in allFileList:
        ...
        target = yourPath + "/" +file
        parser.add_argument("--target", default=target)
        parser.add_argument("--num_paths", type=int, default=100)
        parser.add_argument("--max_width", type=float, default=0.2)
